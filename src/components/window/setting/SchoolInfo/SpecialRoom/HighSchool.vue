        ...
        <li v-for="classroom in classrooms" :key="classroom.id">
          ...
          <ul v-if="connectingStatus[classroom.id]">
            <li v-for="subject in subjects" :key="classroom.id + '-' + subject.id + '-e'">
              ...
              <ul>
                <li v-for="subSubject in subject.subSubjects" :key="subSubject.id">
                  <label>
                    <input type="checkbox" @click="()=>{updateConnection(subSubject.id, classroom.id)}" :checked="subSubject.specialRooms.map(c => c.id).includes(classroom.id)"/>
                    {{ subSubject.name }}
                  </label>
                </li>
              </ul>
          ...
          <div v-else>
            関連教科:&nbsp;
            <span v-for="subject in subjects" :key="classroom.id + '-' + subject.id + '-s'">
              <span v-for="subSubject in subject.subSubjects.filter(s => s.specialRooms.map(c => c.id).includes(classroom.id))" :key="classroom.id + '-' + subject.id + '-' + subSubject.id + '-s'">
                ・{{ subSubject.name }}
              </span>
            </span>
            <br />
...
<script setup>
  ...
  defineProps(["classrooms", "subjects"])
