        ...
        ">
          <template v-for="(unit, index) in [{kw: ['main'], title: '主要教科'}, {kw: ['sub', 'pe'], title: '副教科'}, {kw: ['other'], title: 'その他'}]" :key="index">
            <HighSchoolLargeGroup
              ...
              @remove-subject-from-homeroom="removeSubjectFromHomeroom"
            />
          </template>
        </template>
      </div>
...
<script setup>
  ...
  import HighSchoolLargeGroup from './UnitGroup/HighSchoolLarge.vue';

  defineOptions({name: "AssignmentH"});
  ...
  const removeSubjectFromHomeroom = function(subjectId, subjectName){
    if(currentClassNum.value){
      const result = confirm(`「${subjectName}」を${currentGrade.value}年${currentClassNum.value}組から削除しますか？`);
      if(result){
        emit(
          "updateConfig",
          props
            ...
            .homeroomUtil
            .setByNumber(currentGrade.value, currentClassNum.value)
            .removeSubject(subjectId).toObject()
        );
      }
    }
  }
