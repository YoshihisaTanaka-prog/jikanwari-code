<template>
  ...
  &nbsp;
  <button @click="removeSubject">削除</button>
...
<script setup>
  ...
  const emits = defineEmits(["removeSubject", "editedTeacher", "assignTeacher", "unassignTeacher"]);

  const isShowingDetails = ref(props.assignment.teachers.length == 0);
  ...
  const removeSubject = function(){
    emits("removeSubject", props.assignment.subject.id, props.assignment.subject.name);
  };
